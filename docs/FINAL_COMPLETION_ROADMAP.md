...
| C | `setup/home-assistant-config-examples` | `Setup: add Home Assistant config examples for VelaDial` | New `homeassistant/README.md`, `homeassistant/packages/veladial_example.yaml`, `homeassistant/localtuya_mapping_notes.md`. Update `docs/setup/raspberry_pi_home_assistant_setup.md` to link them. | All values placeholder. No real Tuya keys, Wi-Fi creds, or HA tokens. *Status: opened — see the GitHub PR list for current state.* |
| D | `docs/ui-concept-direction-package` | `Docs: add UI concept direction package for VelaDial` | Four new files under `docs/ui/`: `ui_concept_direction_matrix.md`, `ui_concept_shortlist.md`, `ui_concept_selection_criteria.md`, `ui_concept_notes.md`. Plus one cross-link in `docs/ui/ux_guide_integration_plan.md`. | Direction change vs. earlier "asset intake checklist" plan — covers all 20 concepts in Hardik's matrix with verdicts against locked v1 scope, a top-3 recommendation, and a reusable scoring framework. Preserves 3 pages / 4 presets / no Environment page / wake-only-first / no fusion / no VL53L4CD / no VL53L0X absolutely. *Status: opened — see the GitHub PR list for current state.* |
